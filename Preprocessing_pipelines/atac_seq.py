...
# Arguments:
    ...
    parser.add_argument('-chip', '--chip_seq', required=False, action='store_true',
                        help="turn on this flag if wants to run paired end chip-seq using this script")
    parser.add_argument('--sortByName', required = False, action = 'store_true', help="turn on this flag to generated bam file sorted by readnames")

    args = parser.parse_args()

    print(args.sortByName)
...
# Check the starting format: // use fastq.gz as the standard starting format
    ...
    call_peak(prefix + '_roughuniq_sortedByPosition.bam', args.species,
    ...
    print(f'awk \'$1=="{prefix1}"{{print 100-$3}}\' ./{prefix}_multiqc_data/multiqc_trimmomatic.txt | xargs echo -n -e "\\t"'
    ...
    subprocess.call(f'awk \'BEGIN{{FS="\\t"}}$1=="{prefix1}"{{print $1"\\t"$5"\\t"$8}}\' '
                    f'./{prefix}_multiqc_data/multiqc_fastqc.txt| xargs echo -n > {prefix1}_stats_summary', shell=True)
    subprocess.call(f'awk \'$1=="{prefix1}"{{print 100-$4}}\' ./{prefix}_multiqc_data/multiqc_trimmomatic.txt | xargs echo -n -e "\\t"'
