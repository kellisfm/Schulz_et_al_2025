...
# Check the starting format: // use fastq.gz as the standard starting format
    ...
    print(f'awk \'BEGIN{{FS="\\t"}}$1=="{prefix}"{{print $1"\\t"$14"\\t"100-$19}}\' '
                    f'./{prefix}_multiqc_data/multiqc_fastqc.txt| xargs echo -n >> {prefix}_stats_summary')
    print(f'awk \'$1=="{prefix}"{{print $6}}\' ./{prefix}_multiqc_data/multiqc_trimmomatic.txt | xargs echo -n -e "\\t"'
    ...
    subprocess.call(f'awk \'BEGIN{{FS="\\t"}}$1=="{prefix}"{{print $1"\\t"$14"\\t"100-$19}}\' '
                    f'./{prefix}_multiqc_data/multiqc_fastqc.txt| xargs echo -n >> {prefix}_stats_summary', shell=True)
    subprocess.call(f'awk \'$1=="{prefix}"{{print $6}}\' ./{prefix}_multiqc_data/multiqc_trimmomatic.txt | xargs echo -n -e "\\t"'
