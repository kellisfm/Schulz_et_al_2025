...
# Arguments:
    ...
    parser.add_argument('--star_version', required=False, help="Specify path to use a specific version of STAR")
    parser.add_argument('--minlen', default = 36, required = False, help = "Minimal length required for trimmomatic, default to 36")
    parser.add_argument('--adapters', required = False, help = "Custome adapters fasta file for trimmomatic")
    ####
    # psudoalignment flags:
    parser.add_argument('-ps', '--psuedo', required=False, action="store_true",
                        help="psuedo alignment using salmon")
    parser.add_argument('-ns', '--no_star', required=False, action="store_true",
                        help="Do not run STAR, featureCounts, picard and qualimap QC")
...
# Check for valid arguments:
    ...
    if args.genome_dir:
        if not os.path.exists(args.genome_dir):
            ...
            sys.exit(1)
        if args.star_version is not None:
            STAR = args.star_version # use customized version
        else:
            STAR = HSAP_STAR 
...
# Check for single-end or paired-end:
    if not args.pairend_mode:
        ...
        if args.adapters is not None:
            trim_SE(prefix + '.fastq.gz', args.threads, adapters=args.adapters, minlen = args.minlen)
        else:    
            trim_SE(prefix + '.fastq.gz', args.threads, minlen = args.minlen)
        
        # run fastqc again after trimming
        fastqc(prefix + '.trimmed.fastq.gz', args.threads)

        # if set, run salmon 
        if args.psuedo :
            salmon_se(SALMON_INDEX, prefix, args.threads)
        ...
        if args.adapters is not None:
            trim_PE(prefix1, prefix2, args.threads, adapters = args.adapters, minlen = args.minlen)
        else:
            trim_PE(prefix1, prefix2, args.threads, minlen = args.minlen)
        ...
        if not args.no_star:
            ...
            generate_stats()
            print(f'awk \'BEGIN{{FS="\\t"}}$1=="{prefix1}"{{print $1"\\t"$5"\\t"100-$9}}\' '
                            f'./{prefix}_multiqc_data/multiqc_fastqc.txt| xargs echo -n > {prefix1}_stats_summary')
            print(
                f'awk \'$1=="{prefix1}"{{print 100-$4}}\' ./{prefix}_multiqc_data/multiqc_trimmomatic.txt | xargs echo -n -e "\\t"'
                f'>>{prefix1}_stats_summary')
            print(f'{SAMTOOLS} view -f 2 {prefix}.bam -@ {args.threads} |wc -l|xargs echo -n -e "\\t" '
                            f'>> {prefix1}_stats_summary')
            print(f'{SAMTOOLS} view -q {args.mapq} -f 2 -F 256 {prefix}.bam -@ {args.threads}|wc -l|xargs echo -n -e "\\t"'
                            f'>> {prefix1}_stats_summary')
            print(f'awk \'$1~"{prefix}"{{print $14}}\' ./{prefix}_multiqc_data/multiqc_featureCounts.txt | xargs echo -n -e "\\t"'
            ...
            print(
                ['bash', '-c', f'cat <(echo -e "sample\\traw_reads\\tduplicates_perc\\ttrimmed_perc\\taligned_perc'
                               f'\\tuniq_reads_perc\\texon_aligned_perc\\tmitocondria_reads_perc") <(awk \'BEGIN{{OFS="\\t"}}'
                               f'{{print $1,$2,$3,$4,$5/($2*2)*100,$6/($2*2)*100,$7,$8/($2*2)*100}}\' {prefix1}_stats_summary) '
                               f'> {prefix1}_stats_summary1'])
            print(f'mv {prefix1}_stats_summary1 {prefix1}_stats_summary\n')  # rename it


            subprocess.call(f'awk \'BEGIN{{FS="\\t"}}$1=="{prefix1}"{{print $1"\\t"$5"\\t"100-$9}}\' '
                            f'./{prefix}_multiqc_data/multiqc_fastqc.txt| xargs echo -n > {prefix1}_stats_summary', shell=True)
            subprocess.call(
                f'awk \'$1=="{prefix1}"{{print 100-$4}}\' ./{prefix}_multiqc_data/multiqc_trimmomatic.txt | xargs echo -n -e "\\t"'
                f'>>{prefix1}_stats_summary', shell=True)
            subprocess.call(f'{SAMTOOLS} view -f 2 -F 256 {prefix}.bam -@ {args.threads} |wc -l|xargs echo -n -e "\\t" '
                            f'>> {prefix1}_stats_summary', shell=True)
            subprocess.call(f'{SAMTOOLS} view -q {args.mapq} -f 2 {prefix}.bam -@ {args.threads}|wc -l|xargs echo -n -e "\\t"'
                            f'>> {prefix1}_stats_summary', shell=True)
            subprocess.call(f'awk \'$1~"{prefix}"{{print $14}}\' ./{prefix}_multiqc_data/multiqc_featureCounts.txt | xargs echo -n -e "\\t"'
