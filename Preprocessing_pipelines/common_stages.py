...
HSAP_GEN_DIR="/hpf/largeprojects/mdwilson/genomes/hsap/star_index"
...
HSAP_CHROM_INFO="/hpf/largeprojects/mdwilson/genomes/hsap/ChromInfo.txt"
...
def multiqc(prefix):
    ...
    '''
    # Using python3.6 version of multiqc
    multiqc_cmd = 'module purge;module load python/3.6.3-MultiQC; %s ./%s* -f -n %s_multiqc' %("multiqc", prefix, prefix) + '\n'
    print(multiqc_cmd)
    subprocess.call(multiqc_cmd, shell = True)
...
@utils.formated_output
def trim_SE(fastqgz, threads, adapters=ADAPTERS, minlen = MINLEN):
    '''
...
@utils.formated_output
def trim_PE(prefix1, prefix2, threads, adapters=ADAPTERS_PE, minlen = MINLEN):
    print('%s -jar -Xmx8g %s PE -phred33 -threads %s %s %s %s %s %s %s ILLUMINACLIP:%s:%s \
                    LEADING:%s TRAILING:%s SLIDINGWINDOW:%s MINLEN:%s 2> %s_trimmomatic.log' \
                    % (JAVA, TRIMOMMATIC, str(threads), prefix1 + '.fastq.gz', prefix2 + '.fastq.gz', prefix1 + '_paired.fastq.gz',
                      ...
                      ILLUMINACLIP, LEADING, TRAILING, WINDOWTRIM, minlen, prefix1) + '\n')
    subprocess.call('%s -jar -Xmx8g %s PE -phred33 -threads %s %s %s %s %s %s %s ILLUMINACLIP:%s:%s \
                    LEADING:%s TRAILING:%s SLIDINGWINDOW:%s MINLEN:%s 2> %s_trimmomatic.log' \
                    % (JAVA, TRIMOMMATIC, str(threads), prefix1 + '.fastq.gz', prefix2 + '.fastq.gz', prefix1 + '_paired.fastq.gz',
                      prefix1 + '_unpaired.fastq.gz', prefix2 + '_paired.fastq.gz', prefix2 + '_unpaired.fastq.gz', adapters,
                      ILLUMINACLIP, LEADING, TRAILING, WINDOWTRIM, minlen, prefix1), shell=True)
...
def bam_to_bw(bam, chrom_info, threads, pe=False):
    ...
    bam_to_bedGraph_cmd1 = '%s -bg -ibam %s  %s > %s' % (GENOMECOVERAGEBED, bam, '' if pe else '', bam.replace('.bam', '.bedGraph'))
    ...
    bam_to_bedGraph_cmd2 = '%s -bg -ibam %s -scale %f %s > %s' % (GENOMECOVERAGEBED, bam, 1e6 / int(num),
                                                                 '' if pe else '', bam.replace('.bam', '.bedGraph'))
